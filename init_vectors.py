        ...
        '前端开发工程师': {
            ...
            'skills': {
                'CSS': {
                    ...
                    'difficulty': 1,
                    'dimensions': ['选择器语法/优先级计算/层叠规则/继承机制', '盒模型/BFC机制/外边距合并', 'Flexbox布局/Grid布局', '定位(position/absolute/fixed/relative/sticky)', 'display属性/浮动(float)', '响应式设计/媒体查询/rem/vw单位', 'CSS变量/阴影效果/渐变/滤镜/混合模式', '过渡动画/关键帧动画/2D/3D变换', '伪类伪元素/group-hover/first/last/odd/even', 'CSS新特性(css3/4)']
                },
                'Git': {
                    'desc': '分布式版本控制系统，用于代码管理和团队协作',
                    'difficulty': 1,
                    'dimensions': ['Git安装配置/SSH密钥配置', '工作区/暂存区/仓库/git init/add/commit/status', 'git diff/log/reflog', 'git reset/revert/checkout/branch', 'git merge/rebase', 'git stash/pop', '远程仓库操作/git clone/pull/push/fetch', '分支管理策略/Git Flow工作流', '冲突解决/cherry-pick', '标签管理/子模块', '钩子hooks/Git内部原理']
                },
                'HTML': {
                    'desc': '超文本标记语言，用于定义网页的内容结构和语义，包括标签、属性、表单、多媒体嵌入等',
                    'difficulty': 1,
                    'dimensions': ['HTML标签与属性/DOCTYPE声明/字符实体', '表单/input类型/表单验证', '多媒体嵌入/图片img和srcset/超链接a', '文档结构(html/head/body)/meta标签/link/script标签', '语义化标签(header/nav/main/section/article/aside/footer)', 'SEO优化/可访问性(a11y)/ARIA属性', 'HTML5新标签(canvas/video/audio/svg)', 'table表格/列表ul/ol/dl', 'iframe内联框架']
                },
                'JavaScript': {
                    'desc': '网页脚本语言，用于实现页面交互功能、DOM操作、事件处理、异步请求等动态效果',
                    'difficulty': 1,
                    'dimensions': ['数据类型(值类型/引用类型)/类型转换/Symbol', '原型链/原型继承/闭包', '作用域(var/let/const)/执行上下文/严格模式', '事件循环(event loop)/宏任务/微任务', 'Promise/async/await/Generator/Iterator', 'Map/Set/WeakMap/WeakSet/Proxy/Reflect', 'JSON/正则表达式/模板字符串', '模块化(CommonJS/ES Module)/模块导入导出', 'DOM操作/事件处理/事件委托/冒泡/捕获', 'BOM操作/AJAX/Fetch/Web Storage', '解构赋值/展开运算符/箭头函数/类(class)/错误处理']
                },
                'TypeScript': {
                    'desc': 'JavaScript的超集，提供静态类型检查、接口定义和更好的IDE支持',
                    'difficulty': 1,
                    'dimensions': ['基础类型(string/number/boolean/null/undefined/void/never/any)', '接口interface/类型别名type', '元组tuple/枚举enum/数组类型', '函数类型/类型推断/类型断言', '泛型generic/泛型约束/泛型工具类型(Pick/Omit/Partial/Required/Record/Exclude/Extract)', '交叉类型/联合类型/类型保护/类型守卫', '声明文件d.ts/declare声明', '装饰器/命名空间/namespace模块', '模块解析/tsconfig.json配置/strict模式']
                },
                'npm/yarn/pnpm': {
                    'desc': 'JavaScript包管理工具，用于安装和管理项目依赖',
                    'difficulty': 1,
                    'dimensions': ['包安装(npm install/yarn add/pnpm add)/包卸载(npm uninstall/remove)', '版本管理/semver语义化版本/^/~符号含义', 'package.json配置/package-lock.json/yarn.lock/pnpm-lock.yaml', 'npx执行命令/脚本scripts/npm scripts钩子', '依赖类型(dependencies/devDependencies/peerDependencies)/全局安装-g', 'nvm/n/nvm-windows版本管理/yarn workspace/pnpm软链接', 'npm publish发布包/npm login/logout', 'npm config配置/镜像源切换/缓存管理', '吊装(h lifting)']
                },
                'ES6+': {
                    'desc': '现代JavaScript语法，包括箭头函数、解构赋值、Promise、async/await等',
                    'difficulty': 2,
                    'dimensions': ['let/const变量声明/模板字符串/箭头函数', '解构赋值/展开运算符/剩余参数/默认参数/解构默认值', 'Promise/async/await/Class类/模块导入导出', 'Symbol/迭代器(Iterator)/生成器(Generator)/Iterator协议/for...of循环', 'Set/Map/WeakSet/WeakMap/Symbol.toStringTag', 'Proxy/Reflect/可选链?./空值合并??', '字符串方法(includes/repeat/padStart/padEnd)/数组方法(map/filter/reduce/find/some/every/findIndex/flat/flatMap/includes)', '对象方法(Object.assign/keys/values/entries/fromEntries)', 'BigInt/globalThis/新方法']
                },
                'React': {
                    'desc': 'Facebook开发的组件化UI库，使用虚拟DOM和单向数据流构建复杂的单页应用',
                    'difficulty': 2,
                    'dimensions': ['JSX语法/元素渲染', '组件定义(function/class)/Props传递/事件处理', 'State状态/setState同步异步/批量更新', '生命周期(getDerivedStateFromProps/componentDidMount/componentDidUpdate/componentWillUnmount)', 'Hooks(useState/useEffect/useContext/useReducer/useCallback/useMemo/useRef/useImperativeHandle/useLayoutEffect/useDebugValue)', '自定义Hook/React.memo/React.lazy', '条件渲染/列表渲染key/状态提升/组件复用(组合/继承)', 'Context/Refs转发/Portal/错误边界', 'Suspense/Lazy/Suspense异步加载', '虚拟DOM/Diff算法/Fiber架构/调和(Reconciliation)', 'useRef的两种用法/性能优化/虚拟列表', 'React Router/状态管理(Redux/Zustand/Recoil/Jotai)', 'CSS-in-JS/styled-components/css modules/tailwind/样式方案', 'TypeScript配合/单元测试(Jest/Testing Library)', 'Next.js服务端渲染/静态导出/API Routes/服务端组件(RSC)/流式渲染']
                },
                'Sass/Less': {
                    'desc': 'CSS预处理器，提供变量、嵌套、混合等高级特性',
                    'difficulty': 2,
                    'dimensions': ['变量定义($/@)/Less变量延迟加载', '嵌套规则/父选择器&', '混合mixin/继承extend/占位符选择器%', '函数(function)/运算/颜色函数/Less内置函数', '条件指令(@if/@else/@for/@each/@while)/Less守卫', '插值#{$var}/作用域', '导入(@import/use/@use)/模块化/Sass输出格式', 'Sass地图(Map)/PostCSS配合/预处理器配置']
                },
                'Tailwind CSS': {
                    'desc': '实用优先的CSS框架，通过原子类快速构建自定义设计',
                    'difficulty': 2,
                    'dimensions': ['基础类(颜色/字体/间距)/响应式前缀(sm/md/lg/xl/2xl)', '状态变体(hover/focus/active/disabled)/伪类变体(group-hover/first/last/odd/even)', 'darkMode暗色模式/容器容器', 'Flexbox(flex/flex-col/flex-wrap)/Grid(grid-cols/gap)/间距(m/p/mt/mb/mx/my)/宽高(w/h/min/max)', '位置(absolute/fixed/relative/sticky)/文本(text/leading/tracking)/字体(font/italic/bold)', '颜色文本(bg/text/border)/圆角(rounded/border)/阴影(shadow)/动画(animate)', '自定义配置(tailwind.config.js)/JIT模式/断点定义/主题扩展', '插件开发/与其他框架配合(React/Vue)/PurgCSS/darkMode配置']
                },
                'Vite': {
                    'desc': '新一代前端构建工具，提供极速的开发服务器启动和热更新',
                    'difficulty': 2,
                    'dimensions': ['项目创建(create vite)/配置vite.config.js/别名alias', '环境变量(.env)/CSS预处理/静态资源处理', '开发服务器配置/HMR热更新/watch模式/Server Options', '构建生产(rollup)/代码分割/压缩/Tree Shaking', '按需编译/依赖预构建/PreBundling/依赖缓存', '插件系统/官方插件(vue/react/preact)/CSS代码分割/Chunk分割策略', '移动端构建/多页面应用/服务端渲染/预览模式', '模块联邦/性能优化/TypeScript支持/ESBuild/esbuild配置']
                },
                'Vue': {
                    'desc': '渐进式JavaScript框架，提供响应式数据绑定和组件化开发，易于上手且灵活',
                    'difficulty': 2,
                    'dimensions': ['模板语法/插值{{}}/指令(v-if/v-show/v-for/v-bind/v-on/v-model)', '计算属性computed/侦听器watch/methods方法', '生命周期(created/mounted/updated/destroyed)/生命周期钩子(onMounted等)', '组件注册/Props传递/事件处理($emit/$on)', 'Slot插槽/Provide/Inject/依赖注入(provide/inject)/Refs', '响应式原理(Proxy)/Reactive/Ref/toRefs/toRef/computed/setter', 'WatchEffect/Watch/组件通信/Teleport/Suspense', '异步组件/nextTick/过渡动画(transition)/KeepAlive缓存', 'Vue Router/状态管理(Pinia/Vuex)/组合式API(setup)/setup语法糖(<script setup>)', '自定义指令/插件开发/TypeScript支持/Vue Test Utils/Vite配合', 'SSR(Nuxt)/静态生成/性能优化/Tree Shaking', '虚拟DOM/Diff算法/Suspense并发/错误处理/自定义过滤器/CSS方案']
                },
                'Webpack': {
                    'desc': '前端模块打包工具，支持代码分割、资源优化和开发服务器',
                    'difficulty': 2,
                    'dimensions': ['entry入口/output输出/loader加载器/plugin插件', 'mode模式(development/production)/devServer开发服务器/HMR热模块替换', '代码分割/动态导入()/预取预加载/chunks分割/splitChunks优化', 'Tree Shaking/压缩混淆/source map/文件监听/devtool选项', 'resolve解析/alias别名/extensions扩展名/externals外部依赖', 'HtmlWebpackPlugin/CleanWebpackPlugin/MiniCssExtractPlugin/TerserPlugin', 'CSS抽离/图片处理/字体处理/babel-loader/ts-loader/vue-loader/file-loader/url-loader', '缓存配置/Tree Shaking原理/Module Federation模块联邦/Scope Hoisting作用域提升', '构建优化/体积分析/持久化缓存/loader执行顺序/plugin机制/Tapable', '自定义loader/自定义plugin/配置分离/环境变量/多页面应用/SSR构建/TypeScript支持/Vue/React配合']
                },
                '浏览器兼容性': {
                    'desc': '处理不同浏览器的渲染差异，使用polyfill和特性检测',
                    'difficulty': 2,
                    'dimensions': ['CSS兼容性(caniuse)/caniuse查询', 'JS兼容性/ES6+兼容/Polyfill/Babel转换/core-js/preset-env', '特性检测/新特性检测/浏览器Hack/条件注释', 'CSS Reset/Normalize/前缀(-webkit-/-moz-/-ms-/-o-)', 'Flexbox兼容/Grid兼容/position:sticky兼容/CSS变量兼容', 'fetch polyfill/Promise polyfill/async/await polyfill/Array新方法兼容/Object新方法兼容', 'IE兼容/移动端WebView/X5内核/UC内核/内核识别', 'Browserslist配置/Autoprefixer/垫片方案', '优雅降级/渐进增强/功能模块检测']
                },
                '移动端适配': {
                    'desc': '响应式布局、rem/vw适配、触摸事件处理等移动端开发技术',
                    'difficulty': 2,
                    'dimensions': ['视口配置(viewport)/DPI/PPI/设备像素比/CSS像素/物理像素', 'rem适配/vw/vh适配/lib-flexible方案/postcss-px-to-viewport/媒体查询响应式', 'Flexbox布局/Grid布局', 'touch事件/tap/click区别/300ms延迟/fastclick/点透问题', '安全区域(safe area)/刘海屏适配/touchstart/touchmove/touchend', '手势处理/单指/多指/下拉刷新/上拉加载/滚动穿透', 'position:fixed问题/1px边框', '图片高清适配/iconfont图标/移动端调试/vconsole', 'chrome远程调试/Safari调试/微信调试/viewport适配/分辨率适配', '系统判断(iOS/Android)/版本兼容/APP WebView/WKWebView/UIWebView/JSBridge通信/APP交互']
                },
                '前端性能优化': {
                    'desc': '代码分割、懒加载、缓存策略、资源压缩等性能提升技术',
                    'difficulty': 3,
                    'dimensions': ['加载优化/首屏渲染/FCP/LCP/FID/CLS/TTI', 'DNS预解析/预连接/预加载/懒加载/懒执行/Tree Shaking', '代码分割/动态导入/路由懒加载/组件懒加载/图片懒加载', '资源压缩/Gzip/Brotli压缩/WebP/AVIF/图片压缩优化', 'CSS优化/JS优化/防抖节流/虚拟列表/虚拟DOM优化/key优化', '减少重排重绘/will-change/content-visibility/长列表优化', '缓存策略/强缓存/协商缓存/Cache-Control/ETag/Last-Modified', 'Service Worker缓存/离线缓存/IndexedDB缓存/浏览器缓存/本地存储缓存', 'HTTP/2多路复用/HTTP/3/CDN加速/DNS缓存', '预渲染/SSR/SSG/边缘计算/骨架屏/Loading状态', '动画性能/GPU加速/transform/opacity优化/滚动性能', '事件委托/统计埋点/性能监控/Lighthouse', 'Chrome DevTools/Performance面板/Network面板/Memory面板', '性能指标(RAIL)/Web Vitals/Core Web Vitals']
                }
            }
        },
        '后端开发工程师': {
            ...
            'skills': {
                'Go': {
                    ...
                    'difficulty': 1,
                    'dimensions': ['基础语法/数据类型/流程控制/函数/方法', '接口/结构体/类型断言/类型别名', '并发基础(goroutine)/channel通道/select/context', 'sync包/Mutex/RWMutex/WaitGroup/Once/Map并发安全', '错误处理/defer/panic/recover/错误包装', '包管理(go mod)/泛型/Range循环', '测试/反射/JSON处理/正则表达式', 'HTTP服务/net/http/gin框架/gRPC', 'MySQL驱动/Redis客户端/数据库事务/连接池', '日志/性能分析pprof/内存分配/GC垃圾回收/协程调度/CSP模型', 'time包/io操作/file操作/模板']
                },
                'Java': {
                    'desc': '企业级后端开发主流语言，拥有Spring生态和丰富的类库支持',
                    'difficulty': 1,
                    'dimensions': ['基础语法/数据类型/流程控制', 'OOP(类/对象/封装/继承/多态)/抽象类/接口/内部类', '异常处理/泛型/集合框架(List/Set/Map)/ArrayList/LinkedList/HashMap/HashSet/HashTable', 'Iterator/Stream API/Lambda表达式/方法引用/函数式接口/Optional', '线程基础/Thread/Runnable/ExecutorService/Callable/Future', 'synchronized/volatile/Atomic原子类/并发容器/JUC包/ReentrantLock/CountDownLatch/CyclicBarrier', '线程池/I/O流/NIO/File操作', '网络编程/TCP/UDP/注解/反射/XML/JSON/序列化', '类加载器/JVM内存模型/GC垃圾回收/JVM参数调优', 'Maven/Gradle/单元测试Junit/设计模式']
                },
                'Linux': {
                    'desc': '服务器操作系统，掌握常用命令和系统管理',
                    'difficulty': 1,
                    'dimensions': ['文件命令(ls/cd/mkdir/rm/cp/mv/find/locate)/xargs命令/ln软链接', '文本处理(cat/head/tail/grep/sed/awk/wc/sort/uniq)/管道重定向/正则表达式/三剑客', '用户权限(chmod/chown/useradd/passwd)', '进程(ps/top/pkill/kill/nohup)', '网络命令(ifconfig/ip/netstat/ss/ping/traceroute/curl/wget)', '磁盘(df/du/fdisk/mount/umount)/压缩(tar/gzip/zip/unzip)', '系统服务(systemctl/service)/定时任务(cron/crontab)/计划任务at', 'SSH远程连接/vim/nano', 'shell基础/bash脚本/环境变量/Shell函数/条件判断/循环/脚本调试', '软件包管理(yum/apt/dnf)/日志查看(journalctl/tail)', '内存free/CPUtop/磁盘IOiostat/网络配置', '防火墙(iptables/firewalld)/DNS配置/主机名配置', 'screentmux']
                },
                'MySQL': {
                    'desc': '最流行的开源关系型数据库，用于结构化数据的持久化存储和查询',
                    'difficulty': 1,
                    'dimensions': ['DDL建库建表/DML增删改查/DQL查询/数据类型', '约束(主键/外键/唯一/非空/默认值)', '索引类型(B-Tree/Hash/R-Tree/全文)/普通索引/唯一索引/复合索引', 'Explain执行计划/SQL优化/优化器/ICP/MRR/BKA/索引下推', 'JOIN连接(INNER/LEFT/RIGHT/FULL)/子查询/分组聚合/排序/去重', '事务(ACID)/隔离级别/MVCC/锁(行锁/表锁/间隙锁)', 'redo log/undo log/binlog/慢查询日志', '分页查询/NULL处理/IFNULL/COALESCE/CASE WHEN', '存储过程/触发器/视图/字符集/编码/UTF8MB4', '并发控制/连接池/主从复制/读写分离', '备份恢复/mysqldump/performance_schema/information_schema', '前缀索引/覆盖索引/最左前缀原则']
                },
                'PostgreSQL': {
                    'desc': '功能强大的开源关系型数据库，支持高级数据类型和复杂查询',
                    'difficulty': 1,
                    'dimensions': ['基础SQL/数据类型(数组/JSON/UUID/范围类型)/表创建管理/约束', '索引(B-tree/Hash/GiST/SP-GiST/GIN)/Explain执行计划/查询优化', 'JOIN连接/子查询/CTE公用表表达式/窗口函数/聚合函数', '数组函数/JSON/JSONB/全文搜索', '事务/MVCC/并发控制/锁', '存储过程/函数/触发器/规则/视图/物化视图/继承', '分区表/数组类型操作/自定义类型/枚举类型/域类型', '模式schema/权限控制/序列/ autoincrement/COPY导入导出', 'pg_dump备份/pg_restore恢复/性能监控/pg_stat_activity/pg_stat_statements', '复制配置/流复制/逻辑复制/扩展插件/PostGIS/性能优化']
                },
                'Python': {
                    'desc': '简洁高效的后端开发语言，适合快速开发和数据处理场景',
                    'difficulty': 1,
                    'dimensions': ['基础语法/数据类型/流程控制', '函数定义/参数传递(*args/**kwargs)/匿名函数lambda', '列表/字典/集合推导式/生成器/迭代器', '装饰器/闭包/面向对象(类/对象/继承/多态)', '特殊方法(__init__/__str__/__len__)/魔术方法', '异常处理/模块导入/包管理/pip/venv虚拟环境', '类型注解/数据结构/字符串操作/文件操作', 'JSON/CSV/正则表达式/日期时间处理', '并发编程(threading/asyncio/multiprocessing)/协程async/await/aiohttp/requests', '单元测试unittest/pytest/日志logging', '数据库(psycopg2/pymysql/sqlalchemy)/ORM/Django/Flask/FastAPI', '接口文档OpenAPI/中间件/认证授权/序列化', '网络编程socket/GC垃圾回收/内存管理/GIL全局锁/深拷贝浅拷贝/hashlib/cryptography']
                },
                'RESTful API': {
                    'desc': '基于HTTP的API设计规范，使用JSON进行数据交换',
                    'difficulty': 1,
                    'dimensions': ['HTTP方法(GET/POST/PUT/PATCH/DELETE)/URI规范/资源命名/状态码(2xx/4xx/5xx)', '请求头/响应头/Content-Type/Accept/Authorization', 'JSON数据格式/请求体/响应体', '分页/过滤/排序/字段筛选/API版本控制', '错误响应格式/HATEOAS/幂等性/安全性', 'CORS跨域/缓存控制/Rate Limiting', 'OpenAPI/Swagger/API文档', 'REST约束(Roy Fielding)/Richardson成熟度模型', 'GraphQL对比/gRPC对比/REST API设计原则', 'URL设计/查询参数/路径参数/请求验证/响应封装/统一响应格式', '异常处理/版本管理/认证方式(JWT/Bearer)/token刷新/OAuth2', 'API网关/负载均衡/超时处理/重试机制']
                },
                '单元测试': {
                    'desc': 'JUnit、pytest等测试框架，编写自动化测试保证代码质量',
                    'difficulty': 1,
                    'dimensions': ['测试金字塔(单元/集成/E2E)/TDD测试驱动开发/BDD行为驱动开发', 'JUnit注解(@Test/@Before/@After)/断言assertEquals/assertTrue/异常测试/参数化测试', 'Mockito模拟/Spy/Stub/@Mock/@InjectMocks/测试隔离', 'pytest基础/assert断言/fixture/parametrize参数化/mock/patch', 'Mocking/Stubbing/Spy/unittest框架/TestCase/setUp/tearDown/assert方法/subTest', '测试覆盖/Coverage报告/CI集成/Jenkins集成/GitHub Actions', '测试报告/HTML测试报告/集成测试', '端到端测试/Selenium/Cypress', '测试数据/测试替身/契约测试/性能测试JMeter', '压力测试/冒烟测试/回归测试/探索性测试']
                },
                'Django': {
                    'desc': 'Python全栈Web框架，内置ORM、管理后台、认证系统等完整功能',
                    'difficulty': 2,
                    'dimensions': ['MTV架构/项目创建django-admin startproject/应用app/URLconf', '视图Views/Request/Response/模板Templates/模板语法/模板标签过滤器', 'ORM模型/字段类型/字段选项/查询集QuerySet/过滤器(Filter/Exclude)/Q对象/F表达式/聚合函数', '跨表查询/select_related/prefetch_related/分页', 'Admin后台/Admin自定义', '表单Forms/ModelForm/表单验证', '中间件Middleware/会话Session/Cookie/缓存Cache/消息框架Messages', '认证Auth/用户模型扩展/权限Permission/分组Group/CSRF防护', '静态文件/文件上传/迁移migrate/makemigrations', 'CBV基于类的视图/FBV基于函数的视图/Mixin混合/通用视图', '信号Signals/信号处理器/表单集', 'Django REST Framework/序列化器/视图集/路由Router/分页/过滤/搜索', '认证类/权限类/限流Throttling', 'Celery异步任务/日志配置/settings配置', '部署uWSGI/gunicorn/Nginx反向代理/生产环境配置/安全设置/CORS/国际化i18n']
                },
                'Docker': {
                    'desc': '容器化技术，将应用及其依赖打包为可移植的容器镜像',
                    'difficulty': 2,
                    'dimensions': ['Docker安装/镜像Images/容器Containers/镜像仓库Registry', 'Dockerfile/基础指令(FROM/RUN/COPY/CMD/ENTRYPOINT)/指令优化/层layer缓存/.dockerignore', '构建上下文/镜像分层/多阶段构建', 'Docker Compose/docker-compose.yml/服务编排', '网络模式(bridge/host/none/overlay)/Docker网络/DNS服务发现', '数据卷Volumes/挂载mount/端口映射-p/环境变量/容器互联', '日志驱动/资源限制cpu/memory/健康检查healthcheck/重启策略/容器监控', 'Docker Swarm/Docker Hub/镜像加速器/Harbor私有仓库/镜像安全扫描', 'Dockerfile最佳实践/镜像压缩优化/distroless镜像/alpine镜像', '权限配置/rootless/Seccomp/AppArmor/Docker API/远程管理', 'Docker-machine/Docker registry/镜像tag/镜像推送拉取', 'CI/CD集成/Jenkins集成/GitHub Actions/Kubernetes']
                },
                'Elasticsearch': {
                    'desc': '分布式搜索和分析引擎，用于全文搜索和日志分析',
                    'difficulty': 2,
                    'dimensions': ['索引Index/文档Document/字段类型(text/keyword/numeric/date/geo)/_mapping', '分词器Analyzer/标准分词/IK分词/中文分词/倒排索引', '文档CRUD/bulk批量操作', '搜索查询(match/term/range/bool)/DSL查询/全文搜索/精确查询/过滤filter', '聚合aggregations/桶聚合/指标聚合/嵌套聚合', '分页from/size/深度分页/search_after/scroll游标', '排序sort/高亮highlight/Suggest自动补全', '拼音搜索/同义词搜索/相关性评分/TF-IDF/BM25/boosting/权重调整/相关度调优', '映射mapping/动态映射/显式映射/别名alias/索引模板', '冷热分离/分片shard/副本replica/集群cluster/节点node/脑裂问题/故障转移', '备份恢复/snapshot快照/跨集群搜索', '监控/_cluster/stats/性能优化/ES客户端/连接池/批量写入/写入优化/搜索优化/缓存/segment合并']
                },
                'FastAPI': {
                    'desc': '现代Python Web框架，基于类型提示，自动生成OpenAPI文档',
                    'difficulty': 2,
                    'dimensions': ['路由装饰器@app.get/post/put/delete/路径参数/查询参数', '请求体Pydantic模型/响应模型/状态码/HTTPException异常', '依赖注入/Depends/OAuth2密码模式/JWT认证/安全机制', '异步async def/BackgroundTasks/WebSocket', '中间件/CORS跨域/静态文件/模板Jinja2', '数据库集成/SQLAlchemy/ORM操作/异步数据库/事务/迁移Alembic', '测试/TestClient/pytest/自动文档Swagger/OpenAPI/交互式文档', '参数验证Pydantic/字段校验/嵌套模型/列表字段/可选字段/默认值/枚举类型', 'email/url格式/自定义验证器', 'Response参数/流式响应/文件上传下载/Cookie/Headers', '标签tags/分组路由/批量路由/应用事件startup/shutdown/生命周期', '日志logging/配置config/环境变量', '部署uvicorn/gunicorn/Docker部署/Dockerfile/nginx反向代理/HTTPS配置/性能优化/缓存/限流']
                },
                'GraphQL': {
                    'desc': '灵活的API查询语言，客户端可精确获取所需数据',
                    'difficulty': 2,
                    'dimensions': ['Schema定义/类型系统/ObjectType/字段/标量类型/枚举类型', '接口Interface/联合类型Union/输入类型Input', '查询Query/变更Mutation/订阅Subscription', 'Resolver解析器/字段解析/上下文context/参数传递', '查询验证/查询执行/N+1问题/DataLoader批量加载', '分页/Cursor分页/Offset分页/Relay规范/Connection规范', '碎片Fragment/内省Introspection/变量Variables/指令Directive/内建指令@include/@skip/@deprecated', '错误处理/错误扩展/GraphQL Playground/GraphiQL', 'Apollo Server/Apollo Client/Apollo Federation', 'Schema设计/版本化/认证授权/权限控制/JWT集成/字段级权限', '缓存策略/性能监控/查询复杂度分析/限流', 'Subscriptions/WebSocket/实时推送', '代码生成/类型生成/代码优先schema-first/Schema优先', 'NestJS集成/FastAPI集成/Django集成/Spring Boot集成/批量操作/文件上传']
                },
                'JWT认证': {
                    'desc': 'JSON Web Token，用于无状态的用户身份验证',
                    'difficulty': 2,
                    'dimensions': ['JWT结构(header/payload/signature)/Base64编码/header定义/payload声明', '签名算法HS256/RS256/ES256/HMAC签名/RSA签名/ECDSA签名', '密钥管理/私钥公钥/token生成/token验证/token解析', '过期时间exp/签发时间iat/生效时间nbf/发行者iss/受众aud/主题sub/唯一标识jti', '刷新token/token吊销/黑名单/token续期', '无状态认证/分布式认证/单点登录SSO/OAuth2配合', 'Session对比/Cookie/LocalStorage存储/XSS防护/CSRF防护/CORS配置', 'Authorization Bearer/请求头Authorization', 'token泄露处理/密钥轮换/短期access token/长期refresh token', '安全最佳实践/HSTS/Secure flag/HttpOnly cookie/SameSite/token窃取防护', 'python-jwt/PyJWT/java-jwt/JJWT/Node jsonwebtoken/Go jwt', '登录流程/注册流程/修改密码/JWT最佳实践']
                },
                'Kafka': {
                    'desc': '高吞吐量分布式流处理平台，用于实时数据传输和处理',
                    'difficulty': 2,
                    'dimensions': ['消息队列模式/发布订阅/Topic主题/Partition分区/Replication副本/ISR', 'Leader/Follower/Producer生产者/Consumer消费者/Consumer Group', '消息顺序/消息偏移offset/消费位移/自动提交/手动提交/消费模式', 'Exactly Once/At Least Once/At Most Once', '消息压缩/snappy/gzip/lz4/zstd/消息路由/分区策略/key路由/粘性分区', '幂等性/事务消息/幂等Producer/acks配置/retries重试/拦截器', '序列化/JSON/Avro/Protobuf/Schema Registry', 'Connect连接器/Source Connector/Sink Connector/Stream流处理/Kafka Streams', '状态存储/窗口计算/KSQL', '集群架构/Controller控制器/选举机制/副本分配/再平衡rebalance', '分区分配策略/消费者协调/Coordinator协调器/offset管理', '监控JMX/Kafka Manager/Kafka Eagle/Kafka UI', '性能调优/吞吐量优化/延迟优化/磁盘优化/网络优化', 'Spring Kafka/Python kafka-python/confluent-kafka/Go segment/kafka-go/Java KafkaClient', 'Spring Cloud Stream/Flink集成/Spark Streaming集成', '消费者组偏移量/消息积压/消费者停滞/重复消费/消息丢失']
                },
                'MongoDB': {
                    'desc': '文档型NoSQL数据库，适合存储非结构化和半结构化数据',
                    'difficulty': 2,
                    'dimensions': ['文档Document/集合Collection/数据库Database/BSON格式/ObjectId', '数据类型(字符串/数字/布尔/数组/对象/日期/Null)', '插入insertOne/insertMany/查询find/findOne', '查询条件(eq/ne/gt/gte/lt/lte/in/nin/and/or/not/exists/type)', '投影project/排序sort/分页skip/limit/游标cursor', '更新updateOne/updateMany/upsert/更新操作符($set/$unset/$inc/$push/$pull/$addToSet/$rename/$bit)/数组更新', '删除deleteOne/deleteMany', '聚合管道Aggregation/$match/$group/$project/$sort/$limit/$skip/$lookup/$unwind/$addFields/$replaceRoot/聚合表达式', '索引Index/单字段索引/复合索引/多键索引/文本索引/地理空间索引/唯一索引/稀疏索引/TTL索引/索引管理', 'Explain执行计划/慢查询日志/写入 concern/读取 preference', '副本集Replica Set/主从复制/选举/分片Shard/分片键/哈希分片/范围分片/均衡器/Chunks', '事务Transaction/Sessions/Change Stream', '存储引擎WiredTiger/WiredTiger缓存/压缩', '认证授权/角色RBAC/用户管理/备份mongodump/恢复mongorestore/导入导出mongoimport/mongoexport', '监控/MongoDB Compass/性能优化/连接池/PyMongo/Mongoose/Spring Data MongoDB']
                },
                'Nginx': {
                    'desc': '高性能Web服务器和反向代理，用于负载均衡和静态资源服务',
                    'difficulty': 2,
                    'dimensions': ['安装配置/nginx.conf主配置/全局块/events块/http块/server块/location块', '静态资源服务/反向代理proxy_pass', '负载均衡upstream/轮询round-robin/权重weight/ip_hash/least_conn/keepalive长连接/健康检查', 'HTTP服务器/listen监听端口/server_name域名/root根目录/index默认页/try_files', '重定向rewrite/redirect/permanent/temp/正则rewrite/return/301/302/强制HTTPS/www重定向', 'SSL/TLS配置/HTTPS证书/SSL证书配置/HTTP/2/TLS版本/TLS握手/HSTS/OCSP stapling', 'gzip压缩/gzip_types/gzip_static', '缓存配置/expires/Cache-Control/proxy_cache', '反向代理头proxy_set_header/X-Real-IP/X-Forwarded-For', 'WebSocket代理/proxy_http_version 1.1/Upgrade头/连接升级', '限流limit_req/limit_conn/漏桶算法/令牌桶', 'IP白名单/黑名单/allow/deny', '日志配置/access_log/error_log/日志格式/JSON日志/日志切割/logrotate', '进程管理/信号控制/reload热重载/测试配置nginx -t', '性能优化/worker进程/worker_connections/多核利用/sendfile/tcp_nopush/tcp_nodelay/缓冲区大小', '超时配置/keepalive_timeout/client_header_buffer_size/body大小限制/请求超时/反向代理超时', 'FastCGI/php-fpm/Python/uWSGI/Node.js', 'location优先级/正则匹配/精确匹配/前缀匹配/Rewrite规则/if判断', '安全头/X-Frame-Options/X-Content-Type-Options/X-XSS-Protection/Content-Security-Policy/Referrer Policy', '监控/status模块/Prometheus/lua模块/openresty/stream模块/TCP代理/UDP代理']
                },
                'OAuth2': {
                    'desc': '开放授权协议，支持第三方登录和API访问授权',
                    'difficulty': 2,
                    'dimensions': ['OAuth2角色(Resource Owner/Client/Authorization Server/Resource Server)', '授权流程/授权码Authorization Code/隐式授权Implicit/密码凭证Password/客户端凭证Client Credentials', 'Refresh Token刷新令牌/Access Token访问令牌/Scope范围/State状态参数', 'PKCE防CSRF/code_verifier/code_challenge/code_challenge_method', 'authorization_endpoint/token_endpoint/revocation_endpoint/introspection_endpoint', 'JARM JWT Secured Authorization Response Mode/JWT作为令牌/JWT Bearer/JWKs/jwks_uri', 'JWT验证/JWKS缓存', '授权服务器/客户端注册/client_id/client_secret/redirect_uri', '授权页/登录页/同意页/令牌生成/令牌存储/令牌撤销/令牌刷新/刷新策略', 'GitHub OAuth/Google OAuth/微信OAuth/微博OAuth/钉钉OAuth/支付宝OAuth', 'SSO单点登录/OIDC OpenID Connect/ID Token/UserInfo端点/Discovery文档/jwks_uri端点', 'JWT配置文件/授权服务器实现/Spring Security OAuth2/Authlib/oauthlib', '社交登录/第三方登录/联合登录/登录流程设计', '安全考虑/CSRF防护/重放攻击防护/令牌安全存储/传输安全HTTPS']
                },
                'RabbitMQ': {
                    'desc': '消息队列中间件，实现应用解耦和异步任务处理',
                    'difficulty': 2,
                    'dimensions': ['AMQP协议/消息Message/消息属性(持久化/优先级/TTL/delivery mode)', '队列Queue/队列属性(持久化/排他性/自动删除/死信)', '交换机Exchange/交换机类型(direct/fanout/topic/headers)/绑定Binding/Routing Key路由键', '生产者Publisher/消费者Consumer/basic_publish/basic_consume', 'basic_ack确认/basic_nack拒绝/basic_reject拒绝/手动确认/自动确认', 'QoS预取/prefetch_count/prefetch_size/公平调度', '消息持久化/publisher confirms/事务transaction/发送方确认/mandatory标志', '备份交换机/Alternate Exchange/死信队列DLX/Dead Letter', 'TTL消息过期/队列TTL/延迟队列/延迟插件', '优先级队列/消息顺序/消息优先级/幂等性', '重复消费/消费确认/消费模式/推拉模式/push/pull/多消费者', '集群cluster/镜像队列/仲裁队列Quorum/负载均衡/故障转移/Federation联邦/Shovels', 'Management管理界面/rabbitmqadmin/监控/Prometheus插件/HTTP API/Web管理插件', '虚拟主机vhost/权限控制/用户权限/访问控制', 'Spring AMQP/pika/java-javax-amqp/Go amqp091-go', '消息堆积/消费能力/重试机制/补偿机制/最终一致性/可靠投递/消费幂等/顺序消息/延迟消息/事务消息']
                },
                'Redis': {
                    'desc': '高性能内存键值存储，用作缓存、会话存储和消息队列',
                    'difficulty': 2,
                    'dimensions': ['五种数据类型(String/Hash/List/Set/Sorted Set)', 'String操作(get/set/setnx/mset/mget)', 'Hash操作(hget/hset/hgetall/hkeys/hvals/hincrby)', 'List操作(lpush/rpush/lpop/rpop/lrange/lindex/lset)', 'Set操作(sadd/srem/smembers/sismember/sinter/sunion/sdiff)', 'Sorted Set操作(zadd/zrange/zscore/zrank/zrem)', '过期时间expire/pexpire/ttl/pttl', '键操作(del/exists/rename/type/keys/pattern)/scan遍历/渐进式遍历', '发布订阅pub/sub/subscribe/publish/模式匹配订阅', 'Stream流(XADD/XRANGE/XREAD/XGroup消费者组)', 'Geo地理位置(GEOADD/GEODIST/GEOPOS/GEORADIUS)', 'Bitmaps位图(SETBIT/GETBIT/BITCOUNT/BITOP)', 'HyperLogLog(PFADD/PFCOUNT/PFMerge)', 'Pipeline管道/Lua脚本/Lua EVAL', '事务MULTI/EXEC/WATCH/乐观锁', '持久化RDB/AOF/appendonly/bgrewriteaof/混合持久化', '主从复制/replicaof/masterauth/全量同步/增量同步', '哨兵Sentinel/sentinel monitor/自动故障转移/哨兵选举', 'Redis Cluster/slots槽/hash tags/集群通信/MOVED重定向/ASK重定向', 'failover/resharding/故障检测/集群节点/客户端分片/Codis/Twemproxy', '连接池/Jedis/Lettuce/redis-py/Redisson', '分布式锁/SETNX/SET EX PX NX/锁释放/RedLock算法', '缓存策略(Cache Aside/Read Through/Write Through/Write Behind)', '缓存穿透/布隆过滤器/缓存击穿/互斥锁/永不过期/逻辑过期/缓存雪崩', '内存淘汰策略LRU/LFU/Random/TTL/数据倾斜/内存碎片', '压缩列表/quicklist/ziplist/intset/命令复杂度', '慢查询slowlog/monitor/redis-cli/redis-server/redis-sentinel', 'redis-cli --cluster/cluster meet/cluster slots/槽迁移/key迁移', 'info信息/memory stats/latency/redis-benchmark/bigkeys/memkeys/hotkeys', '安全配置/requirepass/bind/保护模式/ACL/密码配置']
                },
                'Spring Boot': {
                    'desc': 'Java生态的简化开发框架，提供自动配置和开箱即用的微服务支持',
                    'difficulty': 2,
                    'dimensions': ['自动配置Auto Configure/Starter依赖/spring-boot-starter/@SpringBootApplication/main方法/打包jar', '嵌入式服务器Tomcat/Undertow/Jetty', 'application.yml/properties/配置文件加载顺序/环境变量/命令行参数', 'Profile多环境/@ActiveProfile', 'Spring Boot Actuator/actuator端点/health/info/metrics/健康检查/自定义端点', 'Spring Boot DevTools/热重载/LiveReload/依赖管理/parent继承/版本仲裁', 'Web开发/@RestController/@Controller/@RequestMapping/@GetMapping/@PostMapping', '路径变量/查询参数/@RequestParam/@RequestBody/响应体/JSON处理/Jackson/Gson/日期格式化', '国际化i18n/静态资源/webjars/模板引擎Thymeleaf/FreeMarker', '数据访问JdbcTemplate/JPA/Hibernate/MyBatis/Spring Data JPA/Redis/MongoDB', '事务管理@Transactional/声明式事务/编程式事务/传播行为/隔离级别/回滚规则', '异常处理@ExceptionHandler/@ControllerAdvice/全局异常处理/自定义异常/错误页面', '日志配置logback-spring.xml/日志级别/日志格式/日志输出/文件日志/滚动日志', 'Lombok/@Data/@Getter/@Setter/@NoArgsConstructor/@AllArgsConstructor/@Builder/@Slf4j', '安全Spring Security/OAuth2/JWT/认证授权/密码加密/CSRF/CORS', '过滤器Filter/拦截器Interceptor/监听器Listener/启动监听器/ApplicationRunner/CommandLineRunner', '事件机制ApplicationEvent/@EventListener', '异步执行@Async/@EnableAsync/线程池配置/定时任务@Scheduled/@EnableScheduling', '缓存Spring Cache/EhCache/Caffeine/Redis缓存/缓存注解/@Cacheable/@CachePut/@CacheEvict', '单元测试@SpringBootTest/MockMvc/切片测试/WebMvcTest/DataJpaTest', '数据库连接池HikariCP/Druid', '监控Prometheus/Grafana/micrometer', '链路追踪zipkin/Sleuth/SkyWalking', '部署jar/war/Docker/Kubernetes/多环境配置/bootstrap.yml', '配置中心Spring Cloud Config/Nacos/注册中心Eureka/Nacos', '服务发现/客户端负载均衡Feign/OpenFeign', '熔断器Sentinel/Resilience4j/限流/降级']
                },
                'Spring Cloud': {
                    'desc': '微服务架构解决方案，提供服务注册、配置中心、网关等组件',
                    'difficulty': 2,
                    'dimensions': ['微服务架构/服务拆分/服务治理', '服务注册发现Eureka/Nacos/Consul/Zookeeper/心跳机制/自我保护/服务下线', '客户端负载均衡Ribbon/LoadBalancer/RoundRobin/Random/Weighted/Retry', '服务调用Feign/OpenFeign/声明式HTTP客户端', '熔断器Hystrix/Resilience4j/Sentinel/Circuit Breaker/熔断规则/降级处理/限流/流量控制', '配置中心Spring Cloud Config/Nacos/Apollo/配置刷新/热加载/Git配置/SVN配置', '网关Spring Cloud Gateway/Zuul/路由转发/动态路由/路由断言', '认证授权/JWT验证/Token验证/白名单/黑名单/限流/令牌桶/滑动窗口/熔断', '跨域CORS配置/白名单/黑名单/IP限流/限流算法/令牌桶/滑动窗口/漏桶算法', '链路追踪Sleuth/Zipkin/SkyWalking/Jaeger/TraceId/SpanId', '日志聚合ELK/服务监控Actuator/Prometheus/Grafana/监控面板', '分布式事务Seata/AT模式/TCC模式/Saga模式/XA模式', '消息队列RabbitMQ/Kafka/RocketMQ/可靠消息/事务消息/消息确认/消息补偿', '批量消息/延迟消息/顺序消息/幂等性/消息积压/消息丢失', '服务网格Istio/Envoy/Sidecar/流量管理/灰度发布/蓝绿部署/金丝雀发布/A/B测试', '容器化Docker/Kubernetes/Helm/YAML/部署配置/弹性伸缩/滚动更新', '健康检查就绪探针/存活探针/配置环境变量/ConfigMap/Secret', '微服务最佳实践/架构设计/领域驱动设计/CQRS/事件溯源/Saga模式', 'API设计/契约测试Pact/契约验证/文档Swagger/OpenAPI']
                },
                'gRPC': {
                    'desc': '高性能RPC框架，基于Protocol Buffers进行服务间通信',
                    'difficulty': 2,
                    'dimensions': ['Protocol Buffers/.proto文件/消息定义/字段类型/字段编号/默认值/枚举/嵌套消息/导入import', '包名package/选项option/服务定义/rpc方法', '流式RPC/Unary一元/Server Streaming服务端流/Client Streaming客户端流/Bidirectional Streaming双向流', 'HTTP/2/多路复用/头部压缩/双向流/连接复用', 'IDL/proto编译/protoc/protoc-gen-go/protoc-gen-java/protoc-gen-python/生成代码', 'Go gRPC/Java gRPC/Python gRPC/JavaScript gRPC', '服务端开发grpc.Server/RegisterService/Serve/StreamingHandler', '客户端开发grpc.Dial/NewStub/调用方法/阻塞调用/非阻塞调用/流式调用', '元数据Metadata/metadata.New/metadata.Pairs/context传递', '认证TLS/SSL/证书配置/双向TLS/token认证', '拦截器Interceptor/UnaryInterceptor/StreamingInterceptor/服务端拦截器/客户端拦截器', '日志拦截器/监控拦截器/认证拦截器/错误处理status状态码', '重试机制Retry Policy/超时设置deadline/context超时/截止时间', '负载均衡round_robin/pick_first/grpclb/自定义负载均衡', '连接管理keepalive/心跳/健康检查/连接池/Channel池', '反射服务gRPC Reflection/Server Reflection/健康检查服务/ProtoReflection', 'grpcurl/BloomRPC/Postman/拦截器链/验证器/protovalidate/自定义验证', 'JSON映射/JSON转码/HTTP规则/rest transcoding/gRPC-JSON/Swagger文档/OpenAPI导出', '最佳实践命名规范/错误规范/超时规范/重试规范/日志规范/监控规范/Proto风格/向后兼容/可选字段/字段演进']
                }
            }
        },
        '全栈工程师': {
            ...
            'skills': {
                'Git/GitHub': {
                    'desc': '版本控制和代码托管',
                    'difficulty': 1,
                    'dimensions': ['Git安装配置/工作区/暂存区/仓库/git init/add/commit/status', 'git diff/log/reflog/reset/revert/checkout/branch', 'git merge/rebase/stash/pop', '远程仓库操作/clone/pull/push/fetch/SSH密钥配置', '分支管理策略/Git Flow工作流/冲突解决/cherry-pick', '标签管理/子模块/钩子hooks/Git内部原理', 'GitHub Actions/Pull Request/Code Review/分支保护']
                },
                'JavaScript': {
                    'desc': '前后端通用的编程语言，Web开发的核心技术',
                    'difficulty': 1,
                    'dimensions': ['数据类型(值类型/引用类型)/类型转换/原型链/原型继承/闭包', '作用域(var/let/const)/执行上下文/事件循环(event loop)/宏任务/微任务', 'Promise/async/await/Generator/Iterator/Symbol', 'Map/Set/WeakMap/WeakSet/Proxy/Reflect', 'JSON/正则表达式/模板字符串/模块化(CommonJS/ES Module)', 'DOM操作/事件处理/事件委托/冒泡/捕获/BOM操作', 'AJAX/Fetch/Web Storage/解构赋值/展开运算符/箭头函数/类(class)', '模块导入导出/错误处理/严格模式/Node.js基础', 'npm/yarn/pnpm/Buffer/Stream/Crypto/EventEmitter']
                },
                'MySQL/PostgreSQL': {
                    'desc': '关系型数据库，存储应用数据',
                    'difficulty': 1,
                    'dimensions': ['DDL建库建表/DML增删改查/DQL查询/数据类型', '约束(主键/外键/唯一/非空/默认值)', '索引类型(B-Tree/Hash/R-Tree/全文)/普通索引/唯一索引/复合索引', 'Explain执行计划/SQL优化/优化器/索引下推/前缀索引/覆盖索引/最左前缀原则', 'JOIN连接/子查询/分组聚合/排序/去重/NULL处理', '事务ACID/隔离级别/MVCC/锁(行锁/表锁/间隙锁)', 'redo log/undo log/binlog/慢查询日志/分页查询', '存储过程/触发器/视图/字符集/编码/UTF8MB4', '并发控制/连接池/主从复制/读写分离/备份恢复/mysqldump', 'PostgreSQL特有(JSON/JSONB/数组/范围类型)/CTE窗口函数/物化视图/PostGIS/流复制']
                },
                'Node.js': {
                    'desc': 'JavaScript运行时环境，用于服务端开发',
                    'difficulty': 1,
                    'dimensions': ['Node.js架构/模块系统(CommonJS)/事件循环/异步I/O', 'Buffer/Stream流/File System/Crypto加密/EventEmitter', '进程process/child_process/cluster集群/Worker Threads/Timer API', 'http/https模块/net模块/os模块/path模块/querystring/URL处理/zlib压缩', '全局对象(global/window)/模块缓存/模块解析', 'npm/yarn/pnpm/package.json/nvm版本管理', '错误处理/异常捕获/try-catch/uncaughtException/unhandledRejection', '调试debug/Performance API/内存管理/垃圾回收/内存泄漏/CPU泄漏', 'Stream流处理/Transform/Duplex/Readable/Writable/管道pipe', 'Express/Koa/REST API/中间件/路由/模板引擎/静态文件', 'session/cookie/日志/错误处理/安全/CORS/Helmet', '认证授权/JWT/密码加密/bcrypt', '性能优化/进程管理/PM2/集群/负载均衡/Docker部署', '单元测试/集成测试/TypeScript支持/NestJS']
                },
                'RESTful API': {
                    'desc': '设计规范的前后端接口',
                    'difficulty': 1,
                    'dimensions': ['HTTP方法GET/POST/PUT/PATCH/DELETE/URI规范/资源命名/状态码2xx/4xx/5xx', '请求头/响应头/Content-Type/Accept/Authorization', 'JSON数据格式/请求体/响应体', '分页/过滤/排序/字段筛选/API版本控制', '错误响应格式/HATEOAS/幂等性/安全性/CORS跨域/缓存控制/Rate Limiting', 'OpenAPI/Swagger/API文档', 'REST约束Richardson成熟度模型/GraphQL对比/gRPC对比', 'URL设计/查询参数/路径参数/请求验证/响应封装/统一响应格式', '异常处理/版本管理/认证方式token刷新/OAuth2/API网关/负载均衡', '超时处理/重试机制/RESTHooks/HAL/JSON:API']
                },
                'TypeScript': {
                    'desc': '带类型的JavaScript，提升大型项目的可维护性',
                    'difficulty': 1,
                    'dimensions': ['基础类型(string/number/boolean/null/undefined/void/never/any)', '接口interface/类型别名type/元组tuple/枚举enum/数组类型/函数类型', '泛型generic/泛型约束/泛型工具类型(Pick/Omit/Partial/Required/Record/Exclude/Extract)', '交叉类型/联合类型/类型保护/类型守卫/类型断言/类型推断', '声明文件d.ts/命名空间/装饰器/模块解析/tsconfig.json配置/strict模式', 'declare声明/namespace模块/声明合并', '条件类型/映射类型/模板字面量类型/keyof/typeof', 'InstanceType/ReturnType/Parameters/Awaited/NonNullable', 'async/await类型/Promise类型/Observable类型', 'React+TS/Vue+TS/Node+TS/NestJS+TS', '自定义类型守卫/is关键字/类型谓词', 'never类型/unknown/类型收缩/typeof守卫/instanceof守卫/in守卫', 'strictNullChecks/noImplicitAny/strictFunctionTypes/strictPropertyInitialization/noUnusedLocals/noUnusedParameters']
                },
                'Docker': {
                    'desc': '容器化部署应用',
                    'difficulty': 2,
                    'dimensions': ['Docker安装/镜像Images/容器Containers/镜像仓库Registry', 'Dockerfile/基础指令FROM/RUN/COPY/CMD/ENTRYPOINT/指令优化/层layer缓存/.dockerignore', '构建上下文/镜像分层/多阶段构建', 'Docker Compose/docker-compose.yml/服务编排', '网络模式bridge/host/none/overlay/Docker网络/DNS服务发现', '数据卷Volumes/挂载mount/端口映射/环境变量/容器互联/数据持久化', '容器日志/日志驱动/资源限制cpu/memory/健康检查healthcheck/重启策略/容器监控', 'Docker Swarm/Docker Hub/镜像加速器/Harbor私有仓库/镜像安全扫描', 'Dockerfile最佳实践/镜像压缩优化/distroless镜像/alpine镜像', '权限配置/rootless/Seccomp/AppArmor/Docker API/远程管理', 'docker build/pull/push/run/exec/cp/commit/save/load/inspect/stats', 'docker network/volume/镜像tag/镜像推送拉取', 'CI/CD集成/Jenkins集成/GitHub Actions/Kubernetes']
                },
                'Express/Koa': {
                    'desc': 'Node.js Web框架，构建RESTful API服务',
                    'difficulty': 2,
                    'dimensions': ['Express/Koa基础/中间件Middleware/路由Router/请求Request/响应Response/next函数/async中间件', '错误处理/404处理/静态资源/模板引擎/请求体解析/body-parser/JSON解析', 'CORS跨域/Cookie/Session/express-session/cookie-parser', 'JWT认证/路由模块化/MVC模式/数据库集成/MySQL/MongoDB/Redis', 'ORM框架Prisma/TypeORM/查询构造器/事务', '日志Morgan/Winston/错误日志/访问日志/压缩gzip', '缓存/安全Helmet/参数验证joi/validator', '接口文档Swagger/测试jest/supertest/Mock', '部署PM2/Docker/环境配置', 'Koa对比Express/洋葱模型/ctx.context/ctx.request/response', 'koa-body/koa-router/koa-session/koa-cors/koa-compress/koa-logger', '中间件compose/错误处理中间件/日志中间件/CORS中间件', 'Body解析中间件/路由中间件/Gzip压缩/静态文件/模板渲染']
                },
                'Next.js/Nuxt.js': {
                    'desc': '全栈框架，提供服务端渲染和静态生成功能',
                    'difficulty': 2,
                    'dimensions': ['Next.js核心概念/服务端渲染SSR/静态站点生成SSG/增量静态再生成ISR', 'App Router/Pages Router/Link组件/Image组件/Script组件', 'getServerSideProps/getStaticProps/getStaticPaths/getInitialProps', 'API Routes/服务端组件RSC/客户端组件/布局组件Layout/嵌套布局', '样式方案CSS Modules/Sass/styled-jsx/Tailwind/环境变量', '服务端API/API路由/中间件/重定向/rewrite/headers/缓存控制', '性能优化/代码分割/懒加载/Image优化/字体优化/打包分析', 'TypeScript支持/动态路由/路由参数/404处理/错误处理/测试', '部署Vercel/Docker/Node/SSR性能/水合Hydration/Streaming/Suspense', 'Error Boundary/Nuxt.js基础/Nuxt配置/pages目录/layouts目录/components目录', 'composables/自动导入/useRoute/useRouter/useAsyncData/useFetch/useHead', 'SEO优化/meta标签/nuxt generate静态生成/混合渲染', '状态管理Pinia/Vuex/模块系统/插件plugins/中间件middleware', 'CSS方案/全局样式/Tailwind集成/@nuxt/content/@nuxt/image', '内容安全策略/PWA模块/i18n国际化/测试/部署/runtimeConfig/cookie/session', '认证nuxt auth/Prismic/Strapi/directus/Nitro/环保等同构/Tree Shaking']
                },
                'Prisma/TypeORM': {
                    'desc': 'ORM工具，简化数据库操作',
                    'difficulty': 2,
                    'dimensions': ['Prisma基础/schema.prisma/数据模型定义/字段类型', '关系One-to-One/One-to-Many/Many-to-Many/索引/主键/唯一约束/默认约束', 'migrate数据库迁移/migration管理', 'Prisma Client查询CRUD/findUnique/findMany/create/update/delete/upsert', 'connect/disconnect/聚合查询/分页/排序/去重distinct', 'count/sum/avg/min/max/where条件/select字段选择/include关联查询', '嵌套查询/事务/批量操作/Raw查询/原生SQL/错误处理', '连接池配置/连接字符串/环境变量/Prisma Studio/Prisma Accelerate', 'TypeScript支持/生成类型/自动完成/类型安全/Prisma vs TypeORM', 'TypeORM基础/Entity实体/Column/PrimaryColumn/PrimaryGeneratedColumn', 'CreateDateColumn/UpdateDateColumn/ManyToOne/OneToMany/ManyToMany/OneToOne', 'Repository/DataSource/find/findOne/findById/save/create/update/remove/delete', 'createQueryBuilder/QueryBuilder链式调用/select/where/orderBy/groupBy', 'join/leftJoin/rightJoin/relation加载策略/eager/lazy加载/relations预加载', 'SelectIn/SelectJoin/EntityManager/Transaction事务/QueryRunner', 'Migration迁移/EventSubscriber/Subscriber', 'BeforeInsert/AfterInsert/BeforeUpdate/AfterUpdate/BeforeRemove/AfterRemove', '索引管理/多数据源/Redis缓存/日志配置/装饰器/class-validator/class-transformer', '一对一关系/一对多关系/多对多关系/树形结构/Closure Table']
                },
                'React/Vue': {
                    'desc': '前端组件化框架，构建用户界面',
                    'difficulty': 2,
                    'dimensions': ['React基础JSX/组件/Props/State/Hooks(useState/useEffect/useContext/useReducer/useCallback/useMemo/useRef)', '虚拟DOM/Diff算法/Fiber架构/生命周期/组件通信/Context', '状态管理Redux/Zustand/Jotai/Recoil/React Router', '样式方案/styled-components/css modules/tailwind/TypeScript配合', '单元测试Jest/Testing Library/Next.js服务端渲染/静态导出/API Routes', '服务端组件RSC/性能优化useMemo/useCallback/懒加载React.lazy/Suspense', '错误边界/Portals/Refs转发/装饰器模式/合成事件/事件委托', 'setState同步异步/批量更新/PureComponent/React.memo/Profiler', 'Concurrent Mode/Transition/useTransition/useDeferredValue', 'Vue基础模板/指令/响应式/Vue3 Composition API/setup语法糖', 'ref/reactive/computed/watch/watchEffect/生命周期钩子', '组件通信/Slot插槽/Teleport/Suspense/nextTick/过渡动画/KeepAlive缓存', 'Vue Router/Vuex/Pinia/TypeScript/Vite/Nuxt服务端渲染', '组件库ElementUI/Ant Design/Naive UI/Vuetify', '虚拟列表/无限滚动/表单处理/VeeValidate/yup/zod', 'HTTP客户端axios/fetch/国际化i18n/测试', 'E2E测试Cypress/Playwright/代码分割/Tree Shaking/构建优化', 'SSR/SSG/PWA/性能监控/Lighthouse/Core Web Vitals', 'React Query/SWR/TanStack Query/表单管理React Hook Form', '动画Framer Motion/React Spring', '图表ECharts/Recharts/Visx/地图/拖拽国际化react-intl/i18next']
                },
                'Redis': {
                    'desc': '内存数据库，用作缓存和会话存储',
                    'difficulty': 2,
                    'dimensions': ['五种数据类型String/Hash/List/Set/Sorted Set', 'String操作get/set/setnx/mset/mget', 'Hash操作hget/hset/hgetall/hkeys/hvals/hincrby', 'List操作lpush/rpush/lpop/rpop/lrange/lindex/lset', 'Set操作sadd/srem/smembers/sismember/sinter/sunion/sdiff', 'Sorted Set操作zadd/zrange/zscore/zrank/zrem', '过期时间expire/pexpire/ttl/pttl', '键操作del/exists/rename/type/keys/scan遍历', '发布订阅pub/sub/Stream流/Geo地理位置/Bitmaps位图/HyperLogLog', 'Pipeline管道/Lua脚本/事务MULTI/EXEC/WATCH/乐观锁', '持久化RDB/AOF/混合持久化/主从复制/哨兵Sentinel', 'Redis Cluster/slots槽/故障转移/连接池', '分布式锁SETNX/SET EX PX NX/锁释放/RedLock算法', '缓存策略/缓存穿透/击穿/雪崩/布隆过滤器/内存淘汰策略', '慢查询/监控/安全配置/性能优化/BigKey/HotKey', '客户端Jedis/Lettuce/redis-py/go-redis/Jedis/Redisson/ioredis']
                },
                '云服务': {
                    'desc': 'AWS、阿里云等云平台服务使用',
                    'difficulty': 2,
                    'dimensions': ['云服务器ECS/云数据库RDS/对象存储OSS/S3', 'CDN内容分发/负载均衡SLB/弹性伸缩', '容器服务ACK/ECS/Kubernetes服务EKS/ACK', 'Serverless函数计算/API网关/DNS解析/SSL证书', 'WAF防火墙/DDoS防护/日志服务SLS/CloudWatch/监控服务', '配置管理/资源编排/Terraform/CloudFormation', '权限管理IAM/RAM/安全组/网络ACL/VPC专有网络/子网划分/NAT网关/VPN连接/专线', '消息队列MNS/SQS/Kafka托管/Redis托管/MongoDB托管/MySQL托管', 'PostgreSQL托管/Elasticsearch托管', '大数据EMR/数据仓库MaxCompute/数据湖Lakehouse', '机器学习PAI/AI服务/语音识别/图像识别/自然语言处理/内容审核', '视频服务/直播服务/点播服务/移动推送/短信服务/邮件服务', '密钥管理KMS/证书服务/访问控制/审计日志/成本管理/预算告警/资源标签', '成本优化/预留实例/抢占式实例/Savings Plans', '自动化运维/事件总线/定时任务/工作流/流程编排', 'CICD集成/CodePipeline/CodeCommit/CodeBuild/CodeDeploy/容器镜像ECR/ACR', 'Fargate/Lambda/Cold Start/预置并发/Layer/运行时/触发器', '日志查询/分布式追踪/性能监控/应用监控/基础设施监控', '告警规则/通知渠道/故障自愈/自动恢复/多可用区/跨区域容灾/备份恢复/快照/镜像/迁移服务']
                },
                '系统架构': {
                    'desc': '理解单体应用和微服务架构的优缺点',
                    'difficulty': 2,
                    'dimensions': ['单体架构/模块化单体/微服务架构/SOA面向服务', '事件驱动架构EDA/CQRS命令查询职责分离/Event Sourcing事件溯源', 'DDD领域驱动设计/六边形架构/洋葱架构/整洁架构/分层架构/MVC三层架构', '前后端分离/BFF网关/API网关/服务网格/Serverless/云原生架构', '容器化/K8s编排/微服务拆分原则/服务粒度', '服务发现/服务注册/配置中心/分布式事务', 'CAP定理/BASE理论/一致性/可用性/分区容错', '同步调用/异步消息/事件驱动', 'Saga模式/两阶段提交/TCC模式/本地消息表/可靠消息/最终一致性', '高性能/高可用/可扩展性/可维护性/容错性', '负载均衡/限流熔断/降级/隔离/超时控制/重试策略/幂等性', '分布式锁/分布式缓存/分布式Session', 'CDN加速/多级缓存/读写分离/分库分表/数据归档/冷热分离', '搜索架构/日志架构/监控架构/链路追踪/服务监控/健康检查', '优雅上下线/蓝绿部署/金丝雀发布/滚动发布/A/B测试/流量分配/灰度发布/回滚', '弹性伸缩/多租户/SaaS架构/多租户隔离/权限设计/鉴权授权', 'API设计REST/gRPC/GraphQL/服务契约/接口版本/文档管理', '架构演进/重构/评估/权衡/非功能性需求/性能指标/可用性指标/可扩展性指标']
                },
                '性能优化': {
                    'desc': '前后端性能监控和优化策略',
                    'difficulty': 3,
                    'dimensions': ['前端性能指标/加载优化/首屏渲染/FCP/LCP/FID/CLS/TTI', 'DNS预解析/预连接/预加载/懒加载/懒执行/代码分割/动态导入', '路由懒加载/组件懒加载/图片懒加载/Tree Shaking', '资源压缩Gzip/Brotli/图片压缩WebP/AVIF', 'CSS优化/JS优化/防抖节流/虚拟列表/虚拟DOM优化/key优化', '减少重排重绘/will-change/content-visibility/长列表优化', '缓存策略/强缓存/协商缓存/Cache-Control/ETag', 'Service Worker缓存/离线缓存/HTTP/2/3/CDN加速/浏览器缓存/本地存储缓存/IndexedDB缓存', '骨架屏/Loading状态/动画性能/GPU加速/transform/opacity优化/滚动性能/事件委托', '性能监控Lighthouse/Chrome DevTools/Performance面板/Network面板/Memory面板/Web Vitals', '后端性能/数据库优化/索引优化/SQL优化/查询优化/连接池', 'Redis缓存/多级缓存/分布式缓存/本地缓存/热点数据/预热/异步处理', '消息队列/并发优化/线程池/同步转异步/批量处理', '读写分离/分库分表/NoSQL/搜索引擎', 'CDN静态资源/反向代理/负载均衡/限流/熔断/降级/超时优化', 'JVM调优/GC优化/内存优化/CPU优化/IO优化/网络优化', '连接复用/HTTP/2/gRPC/压缩/序列化/API聚合/BFF聚合', 'N+1问题/批量查询/缓存查询/压测Jmeter/Locust/k6', '性能分析profiling/tracing/APM工具SkyWalking/Pinpoint/CAT', '链路追踪/分布式追踪/全链路压测/性能报告/优化建议/效果评估']
                }
            }
        },
        '测试工程师': {
    ...
    for job, info in jobs.items():
        ...
        for skill_name, skill_info in info['skills'].items():
            ...
            if 'dimensions' in skill_info:
                # 按技能存储知识点，用技能名作为key
                skills_dimensions[f"skill_dims_{skill_name}"] = '; '.join(skill_info['dimensions'])
        
        # text 中包含详细知识点维度，便于精确检索
